# Candidate Retrieval 개선 및 재평가 보고서
...
| Validation 구성 | Recall@20 | 평균 target 수 | 평균 오답 target 수 | unknown 후보 생성 |
| --- | --- | --- | --- | --- |
| existing | 98.00% | 0.9676 | 0.0405 | 0.00% |
| osa_short | 100.00% | 0.9865 | 0.0405 | 0.00% |
| osa_short_jamo | 100.00% | 0.9865 | 0.0405 | 0.00% |
| osa_short_jamo_ngram | 100.00% | 0.9865 | 0.0405 | 0.00% |
...
| System | Retrieval Recall@5 | Top-1 | Top-3 | Top-5 | Unknown rejection(빈 pool) | Unknown rejection(정책) | 연구 보류율 | 실제 보류율 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A 기존 retrieval + 거리순위 | 98.68% | 98.24% | 98.68% | 98.68% | 100.00% | 100.00% | 65.59% | 100% |
| 기존 retrieval + LR | 98.68% | 98.24% | 98.68% | 98.68% | 100.00% | 100.00% | 22.67% | 100% |
| B 개선 retrieval + 거리순위 | 100.00% | 99.56% | 100.00% | 100.00% | 100.00% | 100.00% | 65.59% | 100% |
| C 개선 retrieval + LR | 100.00% | 99.56% | 100.00% | 100.00% | 100.00% | 100.00% | 22.67% | 100% |

정답 검색 224→227/227, Top-1 223→226/227. 추가된 정답3건은 모두 수동 검토 대상으로 남는다. LR 적용 전후 Top-1/3/5가 같다. 이름 식별 지표이며 처방 환산 정확도나 임상 정확도가 아니다.

## G. Stress test 설계

별도 `stress.jsonl`: **1388개(known1362 + unknown26)**. target·문자 체계마다 가장 짧은 등록 alias를 결정적으로 선택한80개 원형에 고정 위치 변형을 가했다. 무작위 seed가 필요 없는 생성이며 `make_retrieval_stress.py`로 재현한다. 이미 생성된 파일을 실수로 덮어쓰지 않도록 보호했다.
...
Unknown26개 중23개는 `neurozapine`, `risperidonex`, `뉴로자핀`, `로가핀` 같은 약물 유사 문자열,3개는 무관 문자열이다. 여기서 unknown은 **의도적으로 미등록으로 라벨한 합성 입력**이며 세계 전체에 그런 약물이 없다는 뜻이 아니다. 오타와 미등록명은 문자열만으로 구별할 수 없는 경우가 있다. 같은 입력의 다른 원형·unknown 의도가 겹치는 **6개 행을 삭제하지 않고 `ambiguous`로 표시**했다.

Stress는 추가 난도 시험이며 독립 holdout이 아니다. 사전과 기존 데이터의 성분을 공유하고, fixed test와 완전 동일 original인 stress 행도 34개 있다. 일부 short3/4는 정상 원형 그대로다. 분모는 사례 행이며 동일 이름이 여러 제형/오류 유형에 반복된다. 따라서 전체 평균을 실사용 유병률 가중 성능으로 해석하지 않는다.

## H. Stress test 결과

| System | Retrieval Recall@5 | Top-1 | Top-3 | Top-5 | Unknown rejection(빈 pool) | Unknown rejection(정책) | 연구 보류율 | 실제 보류율 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A 기존 retrieval + 거리순위 | 86.86% | 86.78% | 86.86% | 86.86% | 57.69% | 100.00% | 75.00% | 100% |
| 기존 retrieval + LR | 86.86% | 86.78% | 86.86% | 86.86% | 57.69% | 80.77% | 41.43% | 100% |
| B 개선 retrieval + 거리순위 | 88.03% | 87.96% | 88.03% | 88.03% | 46.15% | 100.00% | 75.00% | 100% |
| C 개선 retrieval + LR | 88.03% | 87.96% | 88.03% | 88.03% | 46.15% | 80.77% | 41.35% | 100% |

선택 개선의 검색 성공은1183→1199/1362, Top-1은1182→1198/1362. 자모를 추가한 별도 진단은 다음과 같다. 이 결과는 선택 정책을 바꾸는 데 사용하지 않았다.

| Stress 채널 구성 | Recall@20 | 검색 성공/1362 | 평균 target 수 | 평균 오답 target 수 | unknown 후보 생성 |
| --- | --- | --- | --- | --- | --- |
| osa_short | 88.03% | 1199 | 0.9179 | 0.0540 | 53.85% |
| osa_short_jamo | 88.69% | 1208 | 0.9258 | 0.0555 | 53.85% |
| osa_short_jamo_ngram | 88.69% | 1208 | 0.9258 | 0.0555 | 53.85% |

자모는 선택 구성 대비 정답9건을 추가 검색하지만 오답 target 총수도75→77로2개 늘었다. ngram은 이번 stress에서도 추가 이득이 없었다.

| Stress 유형 | known N | 기존 검색 성공 | 선택 개선 검색 성공 | +자모 검색 성공 |
| --- | --- | --- | --- | --- |
| delete_replace | 80 | 45 | 45 | 45 |
| deletion | 80 | 69 | 80 | 80 |
| english_duplicate | 53 | 53 | 53 | 53 |
| english_keyboard | 53 | 53 | 53 | 53 |
| english_multiple | 53 | 44 | 44 | 44 |
| formulation_ER | 80 | 80 | 80 | 80 |
| formulation_LAI | 80 | 80 | 80 | 80 |
| formulation_XR | 80 | 80 | 80 | 80 |
| formulation_depot | 80 | 80 | 80 | 80 |
| formulation_injection | 80 | 80 | 80 | 80 |
| formulation_oral | 80 | 80 | 80 | 80 |
| hangul_consonant | 27 | 27 | 27 | 27 |
| hangul_final | 27 | 27 | 27 | 27 |
| hangul_jamo_missing | 27 | 16 | 21 | 27 |
| hangul_two_vowels | 27 | 1 | 1 | 4 |
| hangul_vowel | 27 | 27 | 27 | 27 |
| insert_transpose | 80 | 58 | 58 | 58 |
| insertion | 80 | 80 | 80 | 80 |
| product_formulation_typo | 32 | 0 | 0 | 0 |
| short_hangul_2 | 27 | 0 | 0 | 0 |
| short_hangul_3 | 27 | 22 | 22 | 22 |
| short_hangul_4 | 16 | 15 | 15 | 15 |
| similar_registered_names | 6 | 6 | 6 | 6 |
| space_typo | 80 | 80 | 80 | 80 |
| transposition | 80 | 80 | 80 | 80 |
...
| 데이터 | 시스템 | known N | @1 | @3 | @5 | @10 | @20 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| fixed | A 기존 retrieval + 거리순위 | 227 | 98.24% | 98.68% | 98.68% | 98.68% | 98.68% |
| fixed | 기존 retrieval + LR | 227 | 98.24% | 98.68% | 98.68% | 98.68% | 98.68% |
| fixed | B 개선 retrieval + 거리순위 | 227 | 99.56% | 100.00% | 100.00% | 100.00% | 100.00% |
| fixed | C 개선 retrieval + LR | 227 | 99.56% | 100.00% | 100.00% | 100.00% | 100.00% |
| stress | A 기존 retrieval + 거리순위 | 1362 | 86.78% | 86.86% | 86.86% | 86.86% | 86.86% |
| stress | 기존 retrieval + LR | 1362 | 86.78% | 86.86% | 86.86% | 86.86% | 86.86% |
| stress | B 개선 retrieval + 거리순위 | 1362 | 87.96% | 88.03% | 88.03% | 88.03% | 88.03% |
| stress | C 개선 retrieval + LR | 1362 | 87.96% | 88.03% | 88.03% | 88.03% | 88.03% |
...
| 데이터 | 시스템 | Retrieval error | Ranking error | 검색 성공 시 Top-1 | 정답 검색 평균 rank | 정답 순위 평균 rank |
| --- | --- | --- | --- | --- | --- | --- |
| fixed | A 기존 retrieval + 거리순위 | 3 | 1 | 99.55% | 1.0045 | 1.0045 |
| fixed | 기존 retrieval + LR | 3 | 1 | 99.55% | 1.0045 | 1.0045 |
| fixed | B 개선 retrieval + 거리순위 | 0 | 1 | 99.56% | 1.0044 | 1.0044 |
| fixed | C 개선 retrieval + LR | 0 | 1 | 99.56% | 1.0044 | 1.0044 |
| stress | A 기존 retrieval + 거리순위 | 179 | 1 | 99.92% | 1.0008 | 1.0008 |
| stress | 기존 retrieval + LR | 179 | 1 | 99.92% | 1.0008 | 1.0008 |
| stress | B 개선 retrieval + 거리순위 | 163 | 1 | 99.92% | 1.0008 | 1.0008 |
| stress | C 개선 retrieval + LR | 163 | 1 | 99.92% | 1.0008 | 1.0008 |
...
| 데이터 | 시스템 | 평균 target 수 | median | 최대 | 평균 오답 target 수 | unknown 후보 생성 | 2–3글자 후보 생성 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| fixed | A 기존 retrieval + 거리순위 | 0.9757 | 1 | 2 | 0.0688 | 0.00% | 88.46% |
| fixed | B 개선 retrieval + 거리순위 | 0.9879 | 1 | 2 | 0.0688 | 0.00% | 100.00% |
| stress | A 기존 retrieval + 거리순위 | 0.9049 | 1.0 | 3 | 0.0526 | 42.31% | 63.56% |
| stress | B 개선 retrieval + 거리순위 | 0.9179 | 1.0 | 3 | 0.0540 | 53.85% | 72.06% |
...
| 합성 unknown 입력 | LR 1위 target |
| --- | --- |
| zotapine 1mg QD | zotepine\|non_product_specific |
| risperidonex 1mg QD | risperidone\|non_product_specific |
| olanzapix 1mg QD | olanzapine\|non_product_specific |
| clozapix 1mg QD | clozapine\|non_product_specific |
| paliperidonex 1mg QD | paliperidone\|non_product_specific |
...
- 최종 **232 passed**, skip 없음. 기존 환산/등가표/LAI/parser/UI/모델/고정 데이터와 기존 tests의 원본 해시 50개가 유지된다.
...
원본 dataset SHA256: `0bc7481f554e0268eaf8715ac1ad1f9c8f047ecc1a3504a061f8267984c8d23d`. Stress SHA256: `ba23befe80e1bcad4927869c9e328955e7f8183828d2d6270499ebd8b290adff`. 구현/평가기/validation 해시는 `policy_lock.json`에 있다. Git push와 Render 배포는 수행하지 않았다.
