...
(() => {
  ...
  const labels = {patient:'피험자 ID',date:'처방일',drug:'성분·함량·제형이 적힌 약물명',product:'제품명 (선택)',daily:'하루 정/캡슐 수',days:'처방일수 (선택)'};
  let generation = 0;
  $('rx-file').addEventListener('change', async () => {
    ...
    try {
      ...
      for (const [field, label] of Object.entries(labels)) {
        ...
        select.id = 'map-' + field; title.htmlFor = select.id; title.textContent = label; select.required = !['product', 'days'].includes(field);
  ...
  $('longitudinal-form').addEventListener('submit', async event => {
    ...
    form.append('mode', 'all_dates'); form.append('ack', $('ack').checked ? 'yes' : '');
    $('calculate').disabled = true; $('status').textContent = '환자·처방일별 용량을 계산하고 있습니다…';
    try {
      ...
      $('status').textContent = 'Excel을 받았습니다. Results는 환자·처방일별 결과, MedicationResults는 약물별 상세, Review는 확인할 항목입니다.';
