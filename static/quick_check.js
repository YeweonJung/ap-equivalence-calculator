...
function itemHtml(item, index) {
  if (!item.ok) {
    const suggestions = (item.suggestions || []).map((candidate, candidateIndex) =>
      `<button type="button" class="suggestion-button" data-item="${index}" data-candidate="${candidateIndex}">${escapeHtml(candidate.alias)} (${escapeHtml(candidate.drug)})로 수정 · ${candidate.source === 'llm' ? 'AI 미확인 후보' : `${escapeHtml(candidate.distance)}글자 편집`}<br><small>${escapeHtml(candidate.explanation)}</small></button>`).join(' ');
